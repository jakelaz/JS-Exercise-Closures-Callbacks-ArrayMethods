...
console.log(processSum([], n => n + 1000));
...
function firstNamesAllCaps(runners) {
  const firstNamesAllCapsList = runners.map(element => element.first_name.toUpperCase());
  return firstNamesAllCapsList;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const runnersWithSize = runners.filter(element => element.shirt_size === tShirtSize);
  return runnersWithSize;
}
...
function tallyUpDonations(runners) {
  const sumDonations = runners
  .reduce((p, c) => p + c.donation, 0 );
  return sumDonations;
